...
# Define the square periodic potential
...
V = V0 * np.sign(np.sin(2.0 * np.pi * x / a)) + V0  # Square periodic potential
...
line_potential, = ax.plot(x, V / np.max(V) * np.max(densities), color="red", lw=2, label="Square Periodic Potential")

# Set plot limits and labels
ax.set_xlim(-L/2, L/2)
ax.set_ylim(-2, 1.1 * np.max(densities))
ax.set_title("Wave Packet Dynamics in a Square Periodic Potential")
...
ani.save("wavepacket_square_periodic_potential.gif", fps=30, dpi=150)
