program schrodinger_gaussian
    ...
    integer, parameter :: nx = 500          ! Number of spatial grid points
    integer, parameter :: nt = 1000         ! Number of time steps
    real(dp), parameter :: L = 10.0_dp       ! Length of the box
    ...
    real(dp), parameter :: V0 = 75.0_dp      ! Amplitude of the periodic potential
    real(dp), parameter :: x_a = 1.0_dp       ! Periodicity of the potential
    ...
    k0 = 0.0_dp                             ! Initial wave vector

    ! Set up spatial grid and potential
    do i = 1, nx
        x(i) = -L / 2.0_dp + (i - 1) * dx
        ! Square periodic potential
        V(i) = V0 * sign(1.0_dp, sin(2.0_dp * pi * x(i) / x_a)) + V0
    end do
    ...
    ! Time evolution loop
    open(unit=10, file="wavepacket_animation.dat", status="unknown")
    open(unit=11, file="real_parts_wavefunction.dat", status="unknown")
    open(unit=12, file="imag_parts_wavefunction.dat", status="unknown")
    ...
    do k = 1, nt
        ...
        write(11, '(F10.5)', advance="no") k * dt
        write(12, '(F10.5)', advance="no") k * dt
        do i = 1, nx
            write(11, '(F10.5)', advance="no") real(psi(i))
            write(12, '(F10.5)', advance="no") aimag(psi(i))
        end do
        write(11, *)
        write(12, *)
    end do
