program schrodinger_gaussian
    ...
    x0 = -L / 4.0_dp                        ! Initial position of the wavepacket
    sigma = 0.2_dp                          ! Width of the Gaussian wavepacket
    k0 = 15.0_dp * pi                        ! Initial wave vector

    ! Set up spatial grid and potential
    do i = 1, nx
        x(i) = -L / 2.0_dp + (i - 1) * dx
        V(i) = 0.0_dp                        ! Potential (free particle in a box)
    end do
    ...
    do i = 2, nx-1
        ...
        A(i, i) = 1.0_dp + imag_unit * hbar * dt / (2.0_dp * m * dx**2)
        A(i, i+1) = -imag_unit * hbar * dt / (4.0_dp * m * dx**2)

        B(i, i-1) = imag_unit * hbar * dt / (4.0_dp * m * dx**2)
        B(i, i) = 1.0_dp - imag_unit * hbar * dt / (2.0_dp * m * dx**2)
        B(i, i+1) = imag_unit * hbar * dt / (4.0_dp * m * dx**2)
    end do
