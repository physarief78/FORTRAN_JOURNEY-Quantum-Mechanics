program schrodinger_gaussian
    ...
    real(dp), parameter :: alpha = -10.0_dp  ! Duffing potential quadratic term coefficient
    real(dp), parameter :: beta = 2.0_dp    ! Duffing potential quartic term coefficient
    ...
    x0 = -2.0_dp                             ! Initial position of the wavepacket
    ...
    do i = 1, nx
        x(i) = -L + (i - 1) * dx
        V(i) = 2.0_dp + (alpha * x(i)**2 + beta * x(i)**4)  ! Duffing potential
    end do
    ...
    ! Time evolution loop
    open(unit=10, file="wavepacket_duffing.dat", status="unknown")
    do k = 1, nt
