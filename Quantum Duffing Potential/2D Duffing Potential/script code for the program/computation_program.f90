program schrodinger_gaussian_2d
    ...
    real(dp), parameter :: alpha = -30.0_dp  ! Quadratic term coefficient
    real(dp), parameter :: beta = 25.0_dp   ! Quartic term coefficient
    ...
    x0 = 1.0_dp                          ! Initial x position of the wavepacket
    y0 = -1.0_dp                    ! Initial y position of the wavepacket
    sigma = 0.2_dp                             ! Width of the Gaussian wavepacket
    kx = -1.0_dp * pi                          ! Initial wave vector in x
    ky = 1.0_dp * pi                          ! Initial wave vector in y
    ...
    do i = 1, nx
        do j = 1, ny
            ...
            ! Duffing potential
            V(i, j) = 10.0_dp + (alpha * (x(i, j)**2 + y(i, j)**2) + beta * (x(i, j)**4 + y(i, j)**4))
        end do
    end do
    ...
    ! Time evolution loop
    open(unit=10, file="wavepacket_animation_duffing.dat", status="unknown")
    do k = 1, nt
